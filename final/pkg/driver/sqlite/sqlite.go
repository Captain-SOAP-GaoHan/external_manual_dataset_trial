...
func (drv *Driver) Open() (*sql.DB, error) {
	return sql.Open(driverName, ConnectionString(drv.databaseURL))
}
...
func (drv *Driver) CreateMigrationsTable(db *sql.DB) error {
	_, err := db.Exec(fmt.Sprintf(
		"create table if not exists %s (version varchar(128) primary key, applied_at datetime default current_timestamp)",
	...
	alterStmt := fmt.Sprintf(
		"alter table %s add column applied_at datetime default current_timestamp",
		drv.quotedMigrationsTableName())
