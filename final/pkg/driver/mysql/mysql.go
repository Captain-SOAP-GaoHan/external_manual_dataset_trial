...
func (drv *Driver) CreateMigrationsTable(db *sql.DB) error {
	_, err := db.Exec(fmt.Sprintf(
		"create table if not exists %s (version varchar(128) primary key, applied_at timestamp default current_timestamp)",
	...
	alterStmt := fmt.Sprintf(
		"alter table %s add column if not exists applied_at timestamp default current_timestamp",
		drv.quotedMigrationsTableName())
	_, _ = db.Exec(alterStmt)
...
func (drv *Driver) SelectMigrationsWithDetails(db *sql.DB, limit int) (map[string]*dbmate.MigrationRecord, error) {
	query := fmt.Sprintf("select version, applied_at from %s order by version desc", drv.quotedMigrationsTableName())
	if limit >= 0 {
		query = fmt.Sprintf("%s limit %d", query, limit)
	}
	rows, err := db.Query(query)
	if err != nil {
		// if applied_at column doesn't exist yet, fall back
