...
func (drv *Driver) CreateMigrationsTable(db *sql.DB) error {
	...
	// first attempt at creating migrations table with applied_at column
	createTableStmt := fmt.Sprintf(
		"create table if not exists %s.%s (version varchar primary key, applied_at timestamptz default now())",
		schema, migrationsTable)
	_, err = db.Exec(createTableStmt)
	if err == nil {
		// table exists or created successfully
		// try to add applied_at column if it doesn't exist (for existing tables)
		alterStmt := fmt.Sprintf(
			"alter table %s.%s add column if not exists applied_at timestamptz default now()",
			schema, migrationsTable)
		_ = db.QueryRow(alterStmt).Err()
...
// SelectMigrationsWithDetails returns a list of applied migrations with applied_at timestamps
// with an optional limit (in descending order)
func (drv *Driver) SelectMigrationsWithDetails(db *sql.DB, limit int) (map[string]*dbmate.MigrationRecord, error) {
	migrationsTable, err := drv.quotedMigrationsTableName(db)
	if err != nil {
		return nil, err
	}

	query := "select version, applied_at from " + migrationsTable + " order by version desc"
	if limit >= 0 {
		query = fmt.Sprintf("%s limit %d", query, limit)
	}
	rows, err := db.Query(query)
	if err != nil {
		// if applied_at column doesn't exist yet, fall back to SelectMigrations
		if pqErr, ok := err.(*pq.Error); ok && pqErr.Code == "42703" {
			return drv.selectMigrationsWithDetailsFallback(db, limit)
		}
		return nil, err
	}
	...
	for rows.Next() {
		...
		migrations[version] = &dbmate.MigrationRecord{
			Version:   version,
			AppliedAt: appliedAt,
		}
	}
...
// selectMigrationsWithDetailsFallback returns migration records without applied_at
// for databases where the applied_at column hasn't been added yet
func (drv *Driver) selectMigrationsWithDetailsFallback(db *sql.DB, limit int) (map[string]*dbmate.MigrationRecord, error) {
	basic, err := drv.SelectMigrations(db, limit)
	if err != nil {
		return nil, err
	}

	result := map[string]*dbmate.MigrationRecord{}
	for version := range basic {
		result[version] = &dbmate.MigrationRecord{
			Version:   version,
			AppliedAt: nil,
		}
	}
