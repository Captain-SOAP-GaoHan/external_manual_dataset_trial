...
type DB struct {
	...
	SchemaFile string
	// StatusFormat specifies the output format for the status command ("dashboard", "table", or "json")
	StatusFormat string
...
func (db *DB) FindMigrations() ([]Migration, error) {
	...
	// find applied migrations with details
	...
	if migrationsTableExists {
		appliedMigrations, err = drv.SelectMigrationsWithDetails(sqlDB, -1)
		if err != nil {
			return nil, err
		}
	}
...
func (db *DB) Status(quiet bool) (int, error) {
	...
	switch format {
	case "json":
		return totalPending, db.printStatusJSON(results, totalApplied, totalPending)
	case "table":
		return totalPending, db.printStatusTable(results, totalApplied, totalPending)
	default:
		return totalPending, db.printStatusDashboard(results, totalApplied, totalPending)
	}
}

// StatusJSONMigration represents a single migration in JSON output
...
// StatusJSONOutput represents the complete JSON output for status
...
func (db *DB) printStatusJSON(results []Migration, totalApplied, totalPending int) error {
	output := StatusJSONOutput{
		Total:      len(results),
		Applied:    totalApplied,
		Pending:    totalPending,
		Migrations: make([]StatusJSONMigration, 0, len(results)),
	}
	...
	for _, res := range results {
		...
		if res.AppliedAt != nil {
			s := res.AppliedAt.Format(time.RFC3339)
			m.AppliedAt = &s
		}
	...
	fmt.Fprintln(db.Log, string(data))
	return nil
}

func (db *DB) printStatusTable(results []Migration, totalApplied, totalPending int) error {
	// Table with borders and alignment
	// Header
	fmt.Fprintln(db.Log, "+----------+----------------------+------------------------------------------+---------------------+")
	fmt.Fprintln(db.Log, "| Status   | Version              | Migration Name                           | Applied At          |")
	fmt.Fprintln(db.Log, "+----------+----------------------+------------------------------------------+---------------------+")
	...
	for _, res := range results {
		...
		if res.Applied {
			status = "applied"
			if res.AppliedAt != nil {
				appliedAtStr = res.AppliedAt.Format("2006-01-02 15:04:05")
			} else {
				appliedAtStr = "(unknown)"
			}
		}

		fmt.Fprintf(db.Log, "| %-8s | %-20s | %-40s | %-19s |\n",
			status, res.Version, res.FileName, appliedAtStr)
	}

	fmt.Fprintln(db.Log, "+----------+----------------------+------------------------------------------+---------------------+")
	fmt.Fprintf(db.Log, "Total: %d | Applied: %d | Pending: %d\n", len(results), totalApplied, totalPending)

	return nil
}

func (db *DB) printStatusDashboard(results []Migration, totalApplied, totalPending int) error {
	// Dashboard header
	...
	// Summary section
	fmt.Fprintf(db.Log, "  Total Migrations: %d\n", len(results))
	...
	if len(results) > 0 {
		barWidth := 30
		filled := int(float64(barWidth) * float64(totalApplied) / float64(len(results)))
		bar := ""
		...
		pct := float64(totalApplied) / float64(len(results)) * 100
		fmt.Fprintf(db.Log, "  Progress: [%s] %.0f%%\n", bar, pct)
		fmt.Fprintln(db.Log)
	}
	...
	for _, res := range results {
		...
		fmt.Fprintf(db.Log, "  %s  %-18s %-39s %s\n",
			status, res.Version, res.FileName, appliedAtStr)
	}
